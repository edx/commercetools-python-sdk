...
from ...models.approval_flow import ApprovalFlow, ApprovalFlowUpdate
...
class ByProjectKeyAsAssociateByAssociateIdInBusinessUnitKeyByBusinessUnitKeyApprovalFlowsByIDRequestBuilder:
    ...
    ) -> typing.Optional["ApprovalFlow"]:
        headers = {} if headers is None else headers
        response = self._client._get(
            endpoint=f"/{self._project_key}/as-associate/{self._associate_id}/in-business-unit/key={self._business_unit_key}/approval-flows/{self._id}",
            params={"expand": expand},
            headers=headers,
            options=options,
        )
        if response.status_code == 200:
            return ApprovalFlow.deserialize(response.json())
